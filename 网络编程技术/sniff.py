...
def packet_consumer(pkt):
    ...
    if not pause_sniff_event.is_set():
        ...
        # 推导数据包的协议类型
        proto_names = ['Raw','DNS','TCP', 'UDP', 'ICMP', 'IPv6', 'IP', 'ARP', 'Ether', 'Unknown']
        proto = ''
        ...
        length = len(pkt)
        info = pkt.summary()
        packet_list_tree.insert("", 'end', sniff_count, text=sniff_count, values=(sniff_count, packet_time, src, dst, proto, length, info))
        packet_list_tree.update_idletasks()
# 将抓到的数据包保存为pcap格式的文件
def save_captured_data_to_file():
    fpath = filedialog.asksaveasfilename(defaultextension=".pcap", filetypes=[('pcap files', '.pcap'), ('cap files', '.cap'), ('all files', '.*')])
...
open_button = Button(toolbar, width=8, text="打开数据", command=readPcap)
