...
<body>
	...
	<div class="container">
		<div class="row justify-content-md-center">
			<div class="col col-md-auto question" style="width: calc(100% - 50px);padding: 50px;">
				...
				<table class="table" style="border-bottom: #dee2e6 solid 1px;">
					<tr>
						<td style="width: 66.6%;">内容</td>
						<td style="width: 33.3%;">接收人</td>
					</tr>
					<%
		...
		while(iter1.hasNext())
		{
			Message ma=(Message)iter1.next();
	%>
					<tr>
						<td style="max-width: 66.6%;"><%=ma.getContent() %></td>
						<td style="max-width: 33.3%;"><a
								href="<%=request.getContextPath()%>/CheckUserController?op=none&uname=<%=ma.getPublisher()%>"><%=ma.getPublisher() %></a>
						</td>
					</tr>
					<%
		}
	%>
		...
		while(iter2.hasNext())
		{
			Message ma=(Message)iter2.next();
	%>
					<tr>
						<td style="max-width: 66.6%;"><%=ma.getContent() %></td>
						<td style="max-width: 33.3%;"><a
								href="<%=request.getContextPath()%>/CheckUserController?op=none&uname=<%=ma.getReceiver()%>"><%=ma.getReceiver() %></a>
						</td>
					</tr>
					<%
		}
