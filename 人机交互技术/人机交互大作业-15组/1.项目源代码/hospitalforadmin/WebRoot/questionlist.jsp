...
<html>
  <head>
    ...
    <title>My JSP 'questionlist.jsp' starting page</title>
...
a:active {
  ...
  <body style="background:url(img/蓝色1.jpg);background-size:100%;background-attachment:fixed">
   ...
   <br><div class="main">
   <center>
   <font style="font-size:1.5vw;">问题列表</font>
   <br>
   <table border="1"width="100%" style="table-layout: fixed;" frame="void">
  		<tr style="font-size:1vw;"><th>问题ID</th><th>问题标题</th><th>问题内容</th><th>发布者</th><th>回答数</th><th>科室</th><th></th></tr>
  		<%
  		...
  		%>
  			<tr><td><%=qt.getId()%></td>
  				<td><%=qt.getTitle()%></td>
  				<td><%=qt.getContent()%></td>
  				<td><%=qt.getPublisher()%></td>
  				<td><%=qt.getReplynum()%></td>
  				<td><%=qt.getMajor()%></td>
  				<td><a href="<%=request.getContextPath()%>/QuestionController?op=del&qt=<%=qt.getId()%>">删除</a></td>
